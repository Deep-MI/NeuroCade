...
- `client/`: React 19 + TypeScript UI and Electron desktop launcher. The browser entry point is `src/main.tsx`; reusable UI lives in `src/components/`; pages in `src/pages/`; hooks in `src/hooks/`; shared types and API helpers in `src/types.ts`, `src/constants.ts`, and `src/utils/`. Electron code lives in `client/electron/`.
- `api-service/`: FastAPI backend, assistant orchestration, local runtime tools, and in-process jobs. Browser-facing routers live in `api_service/routers/`. Assistant code lives in `api_service/assistant/`, runtime orchestration in `api_service/runtime/`, runtime tool handlers in `api_service/runtime_tools/`, cases in `api_service/cases/`, and monitoring in `api_service/monitoring/`.
- `backend_common/`: shared backend utilities for settings, auth, database access, providers, storage, workspace bootstrap, and cross-service helpers.
- `packages/neurocade-runtime-tools/`: runtime container request helpers and Apptainer/Docker routing.
...
- Keep unrestricted shell/Python execution out of the web assistant. Container execution should route through approved workflows in `config/neuroimaging_tools.yaml`.
- Worker entrypoints should stay in `api_service.runtime.neuroimaging_tasks`; shared workflow/query/filesystem logic belongs in the corresponding package modules, not in routers.
