...
- `packages/neurocade-runtime-tools/`: Docker runtime request helpers and pinned core tool catalog generation.
...
- `config/` and `scripts/`: project configuration, Docker Compose launchers, install helpers, desktop helpers, release scripts, and admin reset tools.
...
- `.runtime/`, `llm-data/`, and `client/dist/`: generated local runtime/build artifacts. Do not treat these as source unless the task explicitly targets generated state.

## Architecture Overview
The Docker Compose gateway serves the app on `http://localhost:8005`. The main frontend targets `/api/app` as the browser-facing API boundary, while the API service talks to Postgres, Redis, the Docker runtime runner, and the local runtime tools package. The Electron launcher in `client/electron/` wraps the local web app.
...
- `.env`: repository config file for Docker Compose, backend, and token settings. It is not a Python virtualenv.
...
- `./scripts/compose/up.sh -d`: start the local Docker Compose stack required for API and GUI tests.
- `./scripts/compose/images.sh`: build/pull local Docker images and generate the pinned core tool catalog.
...
## Docker Setup Notes
Core services are `gateway`, `api-service`, `api-worker`, `runtime-runner`, `postgres`, and `redis`. Keep `.env` aligned before startup, especially `HOST_DATA_DIR`, `RUNTIME_RUNNER_TOKEN`, `LLM_BACKEND_URL`, Postgres/Redis settings, auth/provider tokens, and `FREESURFER_LICENSE`.
Runtime tool execution is Docker-only. The installed core tool catalog lives under `llm-data/tool-catalog/` and is generated by `./scripts/compose/images.sh`.
The runtime runner should only accept structured Docker container requests from the API service and should mount only approved host data paths.
...
## Testing Guidelines
`pytest` is the test runner; Playwright covers browser flows. Prefer isolated unit tests where possible, reserve Compose-backed runs for API/E2E coverage, and document new setup requirements in `tests/README.md`. GUI and API E2E tests expect the Docker Compose stack on `http://localhost:8005` and seed data under `$HOST_DATA_DIR/output` unless overridden by test-specific environment variables.
