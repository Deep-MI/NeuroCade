...
write_env() {
...

  {
    ...
    env_line APP_NAME "$APP_DISPLAY_NAME"
    env_line APP_BASE_URL "$app_base_url"
    env_line APP_PUBLIC_URL "$app_base_url"
    ...
    env_line LLM_API_TOKEN "$(configured_or_default "$root" LLM_API_TOKEN "$(random_secret)")"
    env_line LLM_PROVIDER_DEFAULT "$provider"
    env_line WORKFLOW_DEFAULT_PROVIDER "$provider"
    env_line LLM_BACKEND_URL "$llm_url"
    env_line LLM_BACKEND_API_KEY "$llm_key"
    env_line LLM_BACKEND_MODEL "$llm_model"
    env_line LLM_NATIVE_TOOL_CALLING "false"
    env_line LLM_JSON_MODE "true"
