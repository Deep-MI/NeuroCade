...
write_env() {
  ...
  local app_base_url app_domain acme_email local_auth client_serve_mode http_bind http_port dashboard_enabled dashboard_insecure dashboard_bind dashboard_port
  ...
  case "$mode" in
    local)
      ...
      if [[ "${NEUROCADE_INSTALL_CHANNEL:-stable}" == "dev" ]]; then
        client_serve_mode="vite"
      fi
    ...
    demo)
      ...
      dashboard_bind="127.0.0.1"
      dashboard_port="8080"
      echo "Demo mode remains rootless: configure your external proxy to forward HTTPS to http://127.0.0.1:$http_port."
      ;;
  esac

  local_auth="$(env_config_value "$root" LOCAL_AUTH_ENABLED "$local_auth")"
  client_serve_mode="$(env_config_value "$root" CLIENT_SERVE_MODE "$client_serve_mode")"
  case "$client_serve_mode" in
    static|vite)
      ;;
    *)
      echo "CLIENT_SERVE_MODE must be static or vite." >&2
      exit 2
      ;;
  esac
  ...
  host_data_dir="$(env_config_value "$root" HOST_DATA_DIR "$host_data_dir")"
  http_bind="$(env_config_value "$root" APP_HTTP_BIND "$http_bind")"
  http_port="$(env_config_value "$root" APP_HTTP_PORT "$http_port")"
  dashboard_enabled="$(env_config_value "$root" TRAEFIK_DASHBOARD_ENABLED "$dashboard_enabled")"
  dashboard_insecure="$(env_config_value "$root" TRAEFIK_API_INSECURE "$dashboard_insecure")"
  dashboard_bind="$(env_config_value "$root" TRAEFIK_DASHBOARD_BIND "$dashboard_bind")"
  dashboard_port="$(env_config_value "$root" TRAEFIK_DASHBOARD_PORT "$dashboard_port")"
  ...
  local redis_password postgres_password llm_api_token
  ...
  tmp_env_path="$(mktemp "$root/.env.tmp.XXXXXX")"
  {
    ...
    env_line CLIENT_SERVE_MODE "$client_serve_mode"
    ...
    env_line_configured "$root" REDIS_SIF "$root/.apptainer/images/redis-7-alpine.sif"
    env_line_configured "$root" TRAEFIK_SIF "$root/.apptainer/images/traefik-v2.11.14.sif"
    env_line_configured "$root" POSTGRES_OCI "docker://postgres:16-alpine"
    env_line_configured "$root" REDIS_OCI "docker://redis:7.2.4-alpine"
    env_line_configured "$root" TRAEFIK_OCI "docker://traefik:v2.11.14"
    ...
    env_line TRAEFIK_DASHBOARD_ENABLED "$dashboard_enabled"
    env_line TRAEFIK_API_INSECURE "$dashboard_insecure"
    env_line TRAEFIK_DASHBOARD_BIND "$dashboard_bind"
    env_line TRAEFIK_DASHBOARD_PORT "$dashboard_port"
    echo
    ...
    env_line_configured "$root" POSTGRES_HOST "127.0.0.1"
    env_line_configured "$root" POSTGRES_PORT "55432"
    env_line_configured "$root" REDIS_HOST "127.0.0.1"
    env_line_configured "$root" REDIS_PORT "56379"
    env_line_configured "$root" API_SERVICE_HOST "127.0.0.1"
    env_line_configured "$root" API_SERVICE_PORT "58080"
    env_line_configured "$root" CLIENT_HOST "127.0.0.1"
    env_line_configured "$root" CLIENT_PORT "5173"
    env_line REDIS_PASSWORD "$redis_password"
    env_line_configured "$root" REDIS_URL "redis://:$redis_password@127.0.0.1:56379/0"
    env_line_configured "$root" DATABASE_URL "postgresql+psycopg://neurocade_user:$postgres_password@127.0.0.1:55432/neurocade_db"
    env_line_configured "$root" API_SERVICE_URL "http://127.0.0.1:58080"
    echo
