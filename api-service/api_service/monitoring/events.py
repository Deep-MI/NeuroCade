...
from backend_common.db import AppEvent, run_with_sqlite_lock_retry
...
_next_retention_cleanup_at = 0.0
...
def _claim_retention_cleanup() -> bool:
    """Claim the process-wide periodic retention cleanup when it is due."""
    global _next_retention_cleanup_at
    now = time.monotonic()
    with _retention_cleanup_lock:
        if now < _next_retention_cleanup_at:
            return False
        _next_retention_cleanup_at = now + RETENTION_CLEANUP_INTERVAL_SECONDS
        return True
...
def record_app_event(
    ...
    *,
    ...
    """Persist a sanitized monitoring event with bounded lock retries."""
...
def record_app_event_best_effort(db: Session, **kwargs: Any) -> AppEvent | None:
    """Record diagnostics without allowing storage failures to break requests."""
    try:
        return record_app_event(db, **kwargs)
    except SQLAlchemyError as exc:
        db.rollback()
        logger.warning("Dropping monitoring event after database failure: %s", exc)
        return None
