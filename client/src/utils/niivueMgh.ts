interface PreparedNiivueVolume {
  buffer: ArrayBuffer;
  filename: string;
}
...
export async function prepareNiivueVolume(buffer: ArrayBuffer, filename: string): Promise<PreparedNiivueVolume> {
  const lowerFilename = filename.toLowerCase();
  if (!lowerFilename.endsWith('.mgz') && !lowerFilename.endsWith('.mgh')) {
    return { buffer, filename };
  }
  const mghBuffer = lowerFilename.endsWith('.mgz') ? await decompressGzip(buffer) : buffer;
  return {
