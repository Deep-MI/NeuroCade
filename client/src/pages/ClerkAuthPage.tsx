...
function LoadingSession() {
  return <div className="nc-app-page px-6 py-10 text-sm text-[var(--nc-tx-muted)]">Loading session...</div>;
}

export function ClerkAuthPage({ mode }: { mode: AuthMode }) {
  ...
  const title = `${APP_DISPLAY_NAME} ${signUp ? 'Sign Up' : 'Sign In'}`;
  ...
  if (localAuthEnabled) {
    return loading ? <LoadingSession /> : <Navigate to={redirectTarget} replace />;
  }
    ...
    return (
      <div className="nc-app-page flex items-center justify-center p-6">
        <div className="nc-card-static w-full max-w-md p-8">
          <h1 className="mb-2 text-2xl font-semibold text-[var(--nc-tx)]">{title}</h1>
  ...
  return (
    <>
      <SignedIn>
        {loading ? <LoadingSession /> : <Navigate to={defaultWorkspacePath} replace />}
      </SignedIn>
      <SignedOut>
        <div className="nc-app-page flex items-center justify-center p-6">
          <div className="nc-card-static w-full max-w-md p-8">
