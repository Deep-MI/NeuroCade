...
function displayCaseId(caseItem: CaseSummary) {
  const prefix = `${caseItem.workspace_id}__`;
  return caseItem.case_id.startsWith(prefix) ? caseItem.case_id.slice(prefix.length) : caseItem.case_id;
}
...
export function CaseCard({
  ...
  const status = STATUS_CONFIG[caseItem.status] ?? STATUS_CONFIG.unknown;
  const isLocked = isRunActive(caseItem.status);
      ...
      {isEditing ? (
        <div className="w-full">
          <div className="mb-3 flex items-start gap-2">
            <input
              data-testid={`workspace-case-rename-input-${caseItem.case_id}`}
              type="text"
              value={editValue}
            ...
            />
            <button type="button" data-testid={`workspace-case-rename-confirm-${caseItem.case_id}`} onClick={() => onRename(caseItem)} disabled={isActionLoading} className="nc-btn nc-btn-active !h-9 !w-9 !p-0">
              {isActionLoading ? <Loader2 size={14} className="animate-spin" /> : <Check size={14} />}
            </button>
            <button type="button" data-testid={`workspace-case-rename-cancel-${caseItem.case_id}`} onClick={onCancelEditing} disabled={isActionLoading} className="nc-btn !h-9 !w-9 !p-0">
              <X size={14} />
            </button>
          </div>
          <span className={`nc-chip ${statusChipClass(caseItem.status)}`}>{status.label}</span>
        </div>
      ) : isDeleting ? (
        <div className="w-full">
          <div className="mb-3 flex items-start justify-between gap-3">
            <div>
              <h3 className="font-semibold text-[var(--nc-tx)]">Delete "{caseItem.subject_name}"?</h3>
              <p className="nc-helper-text mt-1">This removes the case from the workspace.</p>
            </div>
            <span className={`nc-chip ${statusChipClass(caseItem.status)}`}>{status.label}</span>
          </div>
          <div className="flex items-center gap-2">
            <button type="button" data-testid={`workspace-case-delete-confirm-${caseItem.case_id}`} onClick={() => onDelete(caseItem)} disabled={isActionLoading} className="nc-btn nc-chip-red">
              {isActionLoading ? 'Deleting...' : 'Delete'}
            </button>
            <button type="button" data-testid={`workspace-case-delete-cancel-${caseItem.case_id}`} onClick={onCancelDeleting} disabled={isActionLoading} className="nc-btn">Cancel</button>
          </div>
        </div>
      ) : (
        <>
          <div className={cardIsList ? 'flex min-w-0 flex-1 items-center gap-4' : 'flex h-full min-w-0 flex-col'}>
            <div className="min-w-0 flex-1">
              <div className={cardIsList ? '' : 'flex items-start justify-between gap-2'}>
                <button type="button" onClick={(event) => { event.stopPropagation(); onOpen(caseItem); }} className="block min-w-0 cursor-pointer text-left">
                  <h3 data-testid={`workspace-case-title-${caseItem.case_id}`} className="nc-card-title truncate">
                    {caseItem.subject_name}
                  </h3>
                </button>
                {!cardIsList && <span className={`nc-chip shrink-0 ${statusChipClass(caseItem.status)}`}>{status.label}</span>}
              </div>
              <p className="nc-case-secondary-text mt-1 line-clamp-2 text-[var(--nc-tx-muted)]">
                {caseItem.description ?? `Case ID: ${displayCaseId(caseItem)}`}
              </p>
            </div>
            ...
            <div className={`${cardIsList ? '' : 'mt-auto pr-28 pt-3'} flex items-center gap-2`}>
              {cardIsList && <span className={`nc-chip ${statusChipClass(caseItem.status)}`}>{status.label}</span>}
              <span className="nc-case-secondary-text nc-mono text-[var(--nc-tx-faint)]">
                {new Date(caseItem.created_at * 1000).toLocaleDateString()}
              </span>
          ...
          <div className={cardIsList ? 'ml-auto flex shrink-0 items-center gap-1' : 'absolute bottom-2 right-2 flex shrink-0 items-center gap-1 opacity-0 transition group-hover:opacity-100 group-focus-within:opacity-100'}>
            <button type="button" onClick={(event) => { event.stopPropagation(); onStartEditing(caseItem); }} disabled={isLocked || !destructiveActionsEnabled} className="nc-btn !h-8 !w-8 !p-0" title={!destructiveActionsEnabled ? 'Disabled for this deployment' : isLocked ? 'Cannot rename while running' : 'Rename case'} data-testid={`workspace-case-rename-${caseItem.case_id}`}>
              <Pencil size={14} />
            </button>
            ...
            <button type="button" onClick={(event) => { event.stopPropagation(); onStartDeleting(caseItem.case_id); }} disabled={isLocked || !destructiveActionsEnabled} className="nc-btn !h-8 !w-8 !p-0" title={!destructiveActionsEnabled ? 'Disabled for this deployment' : isLocked ? 'Cannot delete while running' : 'Delete case'} data-testid={`workspace-case-delete-${caseItem.case_id}`}>
              <Trash2 size={14} />
            </button>
          </div>
        </>
